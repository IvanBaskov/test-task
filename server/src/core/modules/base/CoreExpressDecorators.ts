    ...
    target.prototype.register = function (router: Application | Router) {
      if (this.routes) {
        ...
        for (let key in this.routes) {
          ...
          console.log(`Registered route: ${route.method} ${url}`);
...
export function AclMiddleware(resource: string, action?: string) {
  return middleware(async (req, res, next) => {
    try {
      if (!req.isAuthenticated()) {
        return next(new ApiAclDenyError(resource, action));
      }
      if (!req.user) {
        return next(new ApiAclDenyError(resource, action));
      }
      ...
      let uid = req.user.id;  // get user id property from express request

      //  Check for administrator
      let isAdministrator = await aclInstance.hasRole(uid, 'Admin');
      if (isAdministrator) {
        return next();
      }

      // Perform permissions check
      let allowed = await aclInstance.isAllowed(uid, resource, action);
      if (allowed) {
        return next();
      } else {
        return next(new ApiAclDenyError(resource, action));
